...
export default async function CashPage({ searchParams }: { searchParams: Promise<{date?:string;store?:string}> }) {
  ...
  return <main className="cash-page">
    ...
    {selected && <CashRegisterEditor key={`${selected.store_id}-${date}-${selected.revision}`} row={selected} categories={(categories || []) as CashCategory[]} people={people || []} editable={date === cashToday()} isAdmin={profile.role === 'admin'}/>}
