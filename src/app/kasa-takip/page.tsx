...
import { getCashData, getCashDescriptionSuggestions } from '@/lib/cash-register-data';
import { sortCashCategories, canEditCashDate, cashToday, cashMoney, cashTotals, type CashCategory } from '@/lib/cash-register';
...
import { CashRegisterFilters } from '@/components/cash-register-filters';
import './cash.css';
...
export default async function CashPage({ searchParams }: { searchParams: Promise<{date?:string;store?:string}> }) {
  ...
  const { date, rows, stores, allStores, profile, canEditHistory } = data;
  ...
  const suggestions = selected ? await getCashDescriptionSuggestions(selected.store_id,date) : {};
  ...
  return <main className="cash-page">
    ...
    <section className="cash-panel cash-toolbar"><CashRegisterFilters date={date} store={params.store || ''} today={cashToday()} allStores={allStores} stores={stores}/><a className="cash-export" href={`/kasa-takip/excel?${query}`}>Excel indir</a><CashRegisterShare rows={rows} date={date}/></section>
    ...
    {selected && <CashRegisterEditor key={`${selected.store_id}-${date}`} row={selected} categories={sortCashCategories((categories || []) as CashCategory[])} suggestions={suggestions} people={people || []} editable={canEditCashDate(date, canEditHistory)}/>}
