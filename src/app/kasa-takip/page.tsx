...
import { canEditCashDate, cashToday, cashMoney, cashTotals, type CashCategory } from '@/lib/cash-register';
...
export default async function CashPage({ searchParams }: { searchParams: Promise<{date?:string;store?:string}> }) {
  ...
  return <main className="cash-page">
    ...
    {selected && <CashRegisterEditor key={`${selected.store_id}-${date}`} row={selected} categories={(categories || []) as CashCategory[]} people={people || []} editable={canEditCashDate(date, canEditHistory)}/>}
