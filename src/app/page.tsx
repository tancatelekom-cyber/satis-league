...
export default async function HomePage() {
  ...
  const monthKey = /^\d{4}-(0[1-9]|1[0-2])$/.test(configuredMonth) ? configuredMonth : currentMonthKey;
  const selectedYear = Number(monthKey.slice(0, 4));
  const selectedMonthIndex = Number(monthKey.slice(5, 7)) - 1;
  const monthLabel = `${MONTH_LABELS[selectedMonthIndex]} ${selectedYear}`;
  const monthStart = toDateString(new Date(selectedYear, selectedMonthIndex, 1));
  const monthEnd = toDateString(new Date(selectedYear, selectedMonthIndex + 1, 0));
