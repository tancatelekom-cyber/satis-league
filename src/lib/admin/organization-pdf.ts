...
function personRow(name: string, accent = false, isOnLeave = false): PdfNode {
  return {
    margin: [0, 0, 0, 5],
    table: {
      widths: ["*"],
      body: [[{
        stack: [
          ...
          ...(isOnLeave
            ? [{ text: "İZİNLİ", bold: true, fontSize: 6.7, color: "#9A6700", margin: [0, 2, 0, 0] }]
            : [])
        ],
...
function storeColumn(store: OrganizationStore, width: number): PdfNode {
  const managers = store.managers.length
    ? store.managers.map((manager) => personRow(manager.full_name, true, manager.is_on_leave))
    : [{ text: "Müdür atanmamış", color: "#8B6B2E", fontSize: 7.5, italics: true, margin: [5, 4, 5, 8] }];
  const employees = store.employees.length
    ? store.employees.map((employee) => personRow(employee.full_name, false, employee.is_on_leave))
          ...
          table: {
            ...
            body: [[{
              stack: [
                ...
                ...(data.coordinator?.is_on_leave
                  ? [{ text: "İZİNLİ", bold: true, fontSize: 7.5, color: "#FFE4A3", margin: [0, 2, 0, 0] }]
                  : data.coordinator
                    ? []
                    : [{ text: "Kullanıcı kaydıyla eşleşmedi", fontSize: 7.5, color: "#DDF4F2", margin: [0, 2, 0, 0] }])
              ],
              fillColor: "#1E5557",
              alignment: "center",
              margin: [10, 8, 10, 8]
