...
export async function requireCashAccess() {
  ...
  const { data: historyPermission } = await createAdminClient().from('feature_profile_permissions').select('can_edit_cash_history').eq('feature_key', 'kasa-takip').eq('profile_id', user.id).maybeSingle();
  return { canEditHistory: historyPermission?.can_edit_cash_history === true, profile, allStores: ['admin', 'management'].includes(profile.role) };
}
