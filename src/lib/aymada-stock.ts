...
const GENERAL_BRANCH_NAME = "Genel stok";
...
function isShowroomCategory(record: XmlRecord) {
  const text = compactText([getFirst(record, ["CategoryName"]), getFirst(record, ["MainCategoryName"])].join(" "));
  return text.includes("TESHIR");
}

function detectCategory(record: XmlRecord): AymadaStockCategory | null {
  if (isShowroomCategory(record)) return null;

  const categoryText = compactText([getFirst(record, ["CategoryName"]), getFirst(record, ["MainCategoryName"])].join(" "));
  const productText = compactText([getFirst(record, ["ProductTypeName"]), getFirst(record, ["ProductCardName"])].join(" "));

  if (categoryText.includes("SMARTPHONE") || categoryText.includes("AKILLI TELEFON")) {
    return "smartphone";
  }
  if (categoryText.includes("TABLET")) return "tablet";
  if (categoryText.includes("IOT") || categoryText.includes("I O T")) return "iot";

  if (productText.includes("SMARTPHONE") || productText.includes("AKILLI TELEFON")) {
    return "smartphone";
  }
  if (productText.includes("TABLET")) return "tablet";
  if (productText.includes("IOT") || productText.includes("I O T")) return "iot";
...
function toStockProduct(record: XmlRecord): AymadaStockProduct | null {
  ...
  return {
    ...
    branchName: GENERAL_BRANCH_NAME,
    category,
    stockCount
  };
}
  ...
  return {
    ...
    warning:
      records.length > 0 && products.length === 0
        ? "Aymada verisi okundu fakat smartphone, tablet veya iot eslesmesi bulunamadi. ProductTypeName, CategoryName ve MainCategoryName alanlari kontrol edilmeli."
