...
export function buildCashPdf(reports: CashReport[]): Promise<Buffer> {
  const content = reports.map((report, index) => {
    ...
    const starts = report.cells.filter(cell => cell.style === 'section').map(cell => cell.row);
    const blocks = starts.map((start, block) => {
    ...
    return {section:blocks, pageSize:'A3', pageOrientation:'landscape', pageMargins:[24,130,24,28],
      header:()=>({margin:[24,24,24,0], table:{widths:Array(report.columns).fill('*'),body:body.slice(0,starts[0]),dontBreakRows:true},layout:blocks[0].layout})};
  });
  return pdfMake.createPdf({pageSize:'A3',pageOrientation:'landscape',pageMargins:[24,130,24,28],defaultStyle:{font:'Roboto',fontSize:8},content,
    ...
    pageBreakBefore:(node:{id?:string;pageNumbers:number[];startPosition?:{top:number}})=>
      Boolean(node.id?.startsWith('cash-block-') && node.pageNumbers.length>1 && (node.startPosition?.top ?? 130)>131)
  }).getBuffer();
}
