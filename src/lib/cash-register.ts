...
export type CashEntry = { category_id: string; category_name?: string; kind?: string; receipt: string; staff: string; staff_name?: string; description: string; cash: number; card: number; payment: 'cash' | 'card' | 'assignment' | 'installment' | 'free'; amount: number };
...
export function calculateCashEntries(entries: CashEntry[], categories: CashCategory[]) {
  const cents = { income: 0, expense: 0, card: 0, assignment: 0, installment: 0 };
  ...
  return { income: cents.income / 100, expense: cents.expense / 100, card: cents.card / 100, assignment: cents.assignment / 100, installment: cents.installment / 100 };
}
