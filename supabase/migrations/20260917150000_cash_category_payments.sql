-- QR ödeme kabul edilir; nakit, gider ve POS toplamlarını etkilemez.
begin;
alter table public.cash_register_categories add column if not exists allowed_payments text[];
update public.cash_register_categories set allowed_payments = array['cash','card','qr','free']::text[]
...
alter table public.cash_register_categories alter column allowed_payments set default array['cash','card','qr','free']::text[];
...
alter table public.cash_register_categories add constraint cash_category_payment_types check (
  ...
  and allowed_payments <@ array['cash','card','qr','assignment','installment','free']::text[]
);
...
begin
  ...
  for entry in select value from jsonb_array_elements(p_entries) loop
    ...
    if entry->>'payment' is null or entry->>'payment' not in ('cash','card','assignment','installment','qr','free') then raise exception 'Geçersiz tahsilat türü.'; end if;
