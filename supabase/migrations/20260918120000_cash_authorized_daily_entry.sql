-- Yetkili müdür ve çalışanlar ilk kayıt dahil kendi şubelerinde bugünün kasasını kaydedebilir.
begin;
alter table public.cash_register_categories add column if not exists allowed_payments text[];
update public.cash_register_categories set allowed_payments = array['cash','card','qr','transfer','free']::text[]
  || case when allow_assignment then array['assignment']::text[] else array[]::text[] end
  || case when allow_installment then array['installment']::text[] else array[]::text[] end
where allowed_payments is null;
alter table public.cash_register_categories alter column allowed_payments set default array['cash','card','qr','transfer','free']::text[];
alter table public.cash_register_categories alter column allowed_payments set not null;
alter table public.cash_register_categories drop constraint if exists cash_category_payment_types;
alter table public.cash_register_categories add constraint cash_category_payment_types check (
  cardinality(allowed_payments) > 0 and array_position(allowed_payments, null) is null
  and allowed_payments <@ array['cash','card','qr','assignment','installment','transfer','free']::text[]
);
