-- Kişi bazlı geçmiş kasa kaydı ekleme/değiştirme. Varsayılan kapalıdır.
begin;
alter table public.feature_profile_permissions
  add column if not exists can_edit_cash_history boolean not null default false;

create or replace function public.cash_register_history_access(target_store uuid)
returns boolean language sql stable security definer set search_path = public as $$
  select public.cash_register_access(target_store) and exists (
    select 1 from public.feature_profile_permissions
    where feature_key = 'kasa-takip' and profile_id = auth.uid() and can_edit_cash_history
  );
$$;
revoke all on function public.cash_register_history_access(uuid) from public;
grant execute on function public.cash_register_history_access(uuid) to authenticated;
...
begin
  ...
  if (p_date is null or p_date > (clock_timestamp() at time zone 'Europe/Istanbul')::date or (p_date < (clock_timestamp() at time zone 'Europe/Istanbul')::date and not public.cash_register_history_access(p_store))) then
    raise exception 'Bu tarihte kasa işlemi yapma yetkiniz yok.';
  end if;
  ...
  if (p_date is null or p_date > (clock_timestamp() at time zone 'Europe/Istanbul')::date or (p_date < (clock_timestamp() at time zone 'Europe/Istanbul')::date and not public.cash_register_history_access(p_store))) then raise exception 'Tarih veya geçmiş işlem yetkisi geçersiz. Sayfayı yenileyin.'; end if;
  ...
  p_cash := 0; p_card := 0; p_transfer := 0; p_expenses := 0; normalized_entries := '[]'::jsonb;
  for entry in select value from jsonb_array_elements(p_entries) loop
    ...
    if entry->>'payment' is null or entry->>'payment' not in ('cash','card','assignment','installment','qr','transfer','free') then raise exception 'Geçersiz tahsilat türü.'; end if;
    if not ((entry->>'payment') = any(category.allowed_payments)) and not exists (
      select 1 from jsonb_array_elements(coalesce(existing.entries,'[]'::jsonb)) old
      where old @> jsonb_build_object('category_id',entry->>'category_id','payment',entry->>'payment',
        'staff',entry->>'staff','receipt',entry->>'receipt','description',entry->>'description','amount',amount)
    ) then raise exception 'Bu kategoride seçilen tahsilat tipi kapalı. İzin verilen bir tip seçin.'; end if;
    ...
    if category.kind = 'income' and entry->>'payment' = 'transfer' then p_transfer := p_transfer + amount; end if;
