...
create table if not exists public.popup_announcements (
  ...
  link_url text,
  image_path text,
  target_mode text not null default 'role',
...
alter table public.popup_announcements
  add column if not exists target_mode text not null default 'role';
...
create table if not exists public.home_leader_settings (
  id uuid primary key default gen_random_uuid(),
  display_month date not null,
  updated_by uuid references public.profiles(id) on delete set null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  check (display_month = date_trunc('month', display_month)::date)
);
